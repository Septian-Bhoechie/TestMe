      ...
      <div class="list-group">
        <a
          ...
          v-bind:class="{
            ...
            active: questionNumber == index + 1,
          }"
          :title="
            hasNotBeenAnswered(index + 1)
              ? 'You have not responded to this question'
              : ''
          "
          @click.prevent="
            storeChoice('random', currentSelection, questionNumber, index)
          "
        >
          Question {{ index + 1 }}
          <span
            v-if="!hasNotBeenAnswered(index + 1)"
            class="badge badge-primary text-light badge-pill float-right"
          >
            <v-icon small class="text-light"> mdi-check </v-icon>
          </span>
          <span
            v-else
            class="badge badge-danger text-light badge-pill float-right"
          >
            <v-icon small class="text-light"> mdi-exclamation </v-icon>
          </span>
        </a>
      </div>
    </div>
    <div class="col-md-10 question-body">
      ...
      <v-card shaped outlined class="p-3" v-else>
        ...
        <v-card-text>
          ...
          <hr />
          <p>
            <v-icon small>mdi-chevron-right</v-icon> This examination will last
            for <strong>{{ examtime }}</strong>
          </p>
          <hr />
          <p>
            <v-icon small>mdi-chevron-right</v-icon> Study every question
            carefully
          </p>
          <hr />
          Best of Luck!
...
export default {
  ...
  methods: {
    ...
    getChosenOption() {
      let selectedArray = this.choices.filter(
        (choice) => choice.question == this.questionNumber
      );
      let selectedOption =
        selectedArray.length > 0 ? selectedArray[0].choice : null;
    ...
    hasNotBeenAnswered(questionNumber) {
      let check = this.choices.filter(
        (choice) => choice.question == questionNumber
      );
      if (check.length > 0) {
    ...
    examtime() {
      ...
      if (this.hours > 0) {
        if (this.hours == 1) {
          this.minutes == 0
            ? (time = "1 hour")
            : (time = `1 hour and ${this.minutes} minutes`);
        } else {
          this.minutes == 0
            ? (time = `${this.hours} hours`)
            : (time = `${this.hours} hours and ${this.minutes} minutes`);
        }
      } else {
